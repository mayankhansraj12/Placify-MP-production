"""
Placify AI - Mock Interview Chat Routes.

RAG-powered question selection plus LLM interviewer for interactive practice.
Sessions are persisted in MongoDB so they survive backend restarts.
"""
...
def _save_session(session: dict) -> None:
    """Persist a session dict to MongoDB."""
    doc = {
        "_id": session["id"],
        "id": session["id"],
    ...
    get_collection("interview_sessions").update_one(
        {"_id": session["id"]},
...
def _load_session(session_id: str) -> Optional[dict]:
    ...
    row = get_collection("interview_sessions").find_one({"_id": session_id})
    if not row:
        return None
    return {
        ...
        "evaluations": _ensure_list(row.get("evaluations")),
        ...
        fallback = [
            {
                ...
                "difficulty": "easy",
                "tip": "Keep it 2 minutes. Past -> Present -> Future format.",
                "skills": [],
            },
            {
                "question": "Describe a challenging project you worked on. What was your role?",
                "difficulty": "medium",
                "tip": "Use STAR method. Quantify the impact.",
                "skills": [],
            },
            {
                "question": "How would you design a URL shortening service?",
                "difficulty": "hard",
                "tip": "Cover: hashing, DB choice, caching, read-heavy optimization.",
                "skills": ["dsa_score"],
            },
            {
                "question": "What is your biggest technical weakness and how are you improving?",
                "difficulty": "easy",
                "tip": "Be genuine. Show self-awareness and growth mindset.",
                "skills": [],
            },
            {
                "question": "Explain the difference between REST and GraphQL APIs.",
                "difficulty": "medium",
                "tip": "Cover trade-offs: over-fetching, schema typing, caching.",
                "skills": ["web_dev_score"],
            },
        ]
        for fb in fallback:
...
async def start_interview(req: StartRequest, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(
        "interview_start session=%s user=%s role=%s questions=%d",
        session_id,
        current_user["id"],
        req.role,
        len(questions),
